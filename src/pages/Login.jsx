import { useState } from 'react';
import { Wallet, Sparkles, Loader2 } from 'lucide-react';
import { useAuth } from '../hooks/useAuth';
import { errorAlert } from '../utils/alerts';

const Login = () => {
  const { signIn } = useAuth();
  const [loading, setLoading] = useState(false);

  const handleGoogleSignIn = async () => {
    setLoading(true);
    try {
      await signIn();
    } catch (err) {
      console.error('Sign in error:', err);
      const errorMessage = err.code === 'auth/popup-closed-by-user'
        ? 'ההתחברות בוטלה. אנא נסה שוב.'
        : err.message || 'אירעה שגיאה בהתחברות. אנא נסה שוב.';
      await errorAlert('שגיאה בהתחברות', errorMessage);
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="login-page" dir="rtl">
      {/* Animated background orbs */}
      <div className="login-orb login-orb-1" />
      <div className="login-orb login-orb-2" />
      <div className="login-orb login-orb-3" />

      {/* Login card */}
      <div className="login-card">
        {/* Brand */}
        <div className="login-brand">
          <div className="login-logo-row">
            <Wallet className="login-wallet-icon" />
            <h1 className="login-title">MyWealth</h1>
            <span className="login-pro-badge">
              <Sparkles size={10} />
              PRO
            </span>
          </div>
          <p className="login-subtitle">ניהול הון אישי חכם</p>
        </div>

        {/* Divider with Google button */}
        <div className="login-divider-row">
          <div className="login-divider-line" />

          <button
            onClick={handleGoogleSignIn}
            disabled={loading}
            className="login-google-btn"
            id="google-sign-in-btn"
            aria-label="התחבר עם Google"
          >
            {loading ? (
              <Loader2 size={20} className="animate-spin" style={{ color: '#94a3b8' }} />
            ) : (
              <svg className="login-google-icon" viewBox="0 0 24 24" width="20" height="20">
                <path
                  fill="#4285F4"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="#34A853"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="#FBBC05"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                  fill="#EA4335"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
              </svg>
            )}
          </button>

          <div className="login-divider-line" />
        </div>

        <p className="login-hint">התחבר עם חשבון Google</p>

        {/* Footer */}
        <p className="login-footer">
          בכניסה אתה מסכים ל
          <a href="/legal/terms" className="login-link">תנאי השימוש</a>
          {' '}ו
          <a href="/legal/privacy" className="login-link">מדיניות הפרטיות</a>
        </p>
      </div>
    </div>
  );
};

export default Login;
