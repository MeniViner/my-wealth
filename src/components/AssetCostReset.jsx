...
import { RotateCcw, Search, AlertCircle, Check, RefreshCw, Loader2 } from 'lucide-react';
import { confirmAlert, successToast } from '../utils/alerts';

/**
 * רכיב לניהול ושחזור עלויות מקוריות של נכסים
 */
const AssetCostReset = ({ assets, onUpdateAsset, isLoading = false, currencyRate, onRefreshPrices }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [resettingAssets, setResettingAssets] = useState(new Set());
    const [previewCurrency, setPreviewCurrency] = useState('ILS'); // 'ILS' or 'USD'
    ...
    /**
     * המרת מטבע - helper function (synchronous for display)
     */
    const convertCurrencySync = (amount, fromCurrency, toCurrency) => {
        if (fromCurrency === toCurrency) return amount;
        const rate = currencyRate?.rate || 3.65;
    ...
    const handleRefreshPrices = async () => {
        ...
        try {
            ...
            await successToast('מחירים עודכנו בהצלחה', 1500);
        } catch (error) {
            console.error('Error refreshing prices:', error);
            await confirmAlert('שגיאה', 'אירעה שגיאה בעדכון המחירים', 'error');
        } finally {
    ...
    /**
     * חישוב ערכים מומרים לכל נכס (preview only)
     */
    const assetsWithConvertedValues = useMemo(() => {
        return filteredAssets.map(asset => {
            ...
            const value = asset.value || (asset.quantity || 0) * currentPrice;
            const originalValue = asset.originalValue || (asset.quantity || 0) * purchasePrice;
            ...
            if (previewCurrency !== assetCurrency) {
                displayCurrentPrice = convertCurrencySync(currentPrice, assetCurrency, previewCurrency);
                displayPurchasePrice = convertCurrencySync(purchasePrice, assetCurrency, previewCurrency);
                displayValue = convertCurrencySync(value, assetCurrency, previewCurrency);
                displayOriginalValue = convertCurrencySync(originalValue, assetCurrency, previewCurrency);
                displayProfitLoss = convertCurrencySync(profitLoss, assetCurrency, previewCurrency);
            }
    ...
    }, [filteredAssets, previewCurrency, currencyRate]);
    ...
    return (
        <div className="space-y-4">
            {/* כפתורי בקרה */}
            <div className="flex flex-wrap items-center justify-between gap-3 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-300">תצוגה מקדימה:</span>
                    <div className="flex items-center gap-1 bg-white dark:bg-slate-800 rounded-lg border border-slate-300 dark:border-slate-600 p-1">
                        <button
                            onClick={() => setPreviewCurrency('ILS')}
                            className={`px-3 py-1.5 text-sm font-medium rounded transition-colors ${previewCurrency === 'ILS'
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 text-sm font-medium rounded transition-colors ${previewCurrency === 'USD'
                                ? 'bg-emerald-600 text-white'
                                : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'
                                }`}
                        >
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400">(תצוגה מקדימה בלבד)</span>
                </div>
                {onRefreshPrices && (
                    <button
                    ...
                    >
                        ...
                        {isRefreshing ? 'מעדכן...' : 'רענן מחירים'}
                    </button>
                )}
            </div>
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                            {filteredAssets.length === 0 ? (
                                ...
                                assetsWithConvertedValues.map((asset) => {
                                    const isResetting = resettingAssets.has(asset.id);
                                    const profitLoss = asset.displayProfitLoss || 0;
                                    const profitLossPercent = asset.profitLossPercent || 0;
                                    const isProfit = profitLoss > 0;
                                    const isLoss = profitLoss < 0;
                                    const currencySymbol = asset.displayCurrency === 'USD' ? '$' : '₪';
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex flex-col">
                                                    <span className="text-lg font-bold text-slate-900 dark:text-slate-100">
                                                        ...
                                                        ) : asset.displayCurrentPrice ? (
                                                            `${currencySymbol}${asset.displayValue.toFixed(2)}`
                                                        ) : (
                                                            'לא זמין'
                                                        )}
                                                    </span>
                                                    <div className="flex items-center gap-2 mt-0.5">
                                                        {isLoading && !asset.displayCurrentPrice ? (
                                                            <div className="animate-pulse flex items-center gap-2">
                                                            ...
                                                            <>
                                                                <span className="text-xs text-slate-500 dark:text-slate-400">
                                                                    {currencySymbol}{asset.displayCurrentPrice.toFixed(2)} למניה
                                                                </span>
                                                                {asset.hasLivePrice && (
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-lg font-bold text-slate-900 dark:text-slate-100">
                                                        {isLoading && (!asset.quantity || !asset.purchasePrice) ? (
                                                            ...
                                                            `${currencySymbol}${asset.displayOriginalValue.toFixed(2)}`
                                                        )}
                                                    </span>
                                                    <span className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                                        {isLoading && !asset.displayPurchasePrice ? (
                                                            <div className="animate-pulse flex items-center gap-2">
                                                                <div className="w-16 h-3 bg-slate-200 dark:bg-slate-700 rounded"></div>
                                                            </div>
                                                        ) : (
                                                            `${currencySymbol}${asset.displayPurchasePrice.toFixed(2)} למניה`
                                                        )}
                                                    </span>
                                                </div>
                                            </td>
                                                    ...
                                                    {isLoading && (asset.profitLoss === null || asset.profitLoss === undefined) ? (
                                                        ...
                                                        <>
                                                            <span className={`font-medium ${isProfit
                                                                ...
                                                                {profitLoss > 0 ? '+' : ''}{currencySymbol}{profitLoss.toFixed(2)}
                                                            </span>
                                                            <span className={`text-xs ${isProfit
                                                                ? 'text-green-600 dark:text-green-400'
                                                                : isLoss
                                                                    ? 'text-red-600 dark:text-red-400'
                                                                    : 'text-slate-600 dark:text-slate-400'
