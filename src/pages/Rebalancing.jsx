...
const GROUP_TYPES = {
  ...
  subcategory: { label: 'קטגוריות חלוקה', icon: Layers, color: '#ec4899' },
...
const Rebalancing = ({ assets, systemData, user, currencyRate, portfolioContext = "" }) => {
  ...
  const getAvailableItems = (groupType) => {
    ...
    switch (groupType) {
      ...
      case 'subcategory':
        return systemData.subcategories || [];
      case 'platform':
    ...
    availableItems.forEach(item => {
      ...
      switch (group.type) {
        case 'category':
          ...
          break;
        case 'subcategory':
          matchingAssets = assets.filter(a => a.subcategory === itemName);
          break;
        case 'platform':
...
`;
  ...
  const getItemColor = (group, item) => {
    ...
    if (group.type === 'subcategory') {
      return systemData?.subcategories?.find(c => c.name === item)?.color || '#94a3b8';
    }
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-8 md:pb-12" dir="rtl">
      ...
      <div className="flex gap-2 border-b border-slate-200 dark:border-slate-700">
        <button
          ...
          className={`px-6 py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'rebalancing'
        ...
        <button
          ...
          className={`px-6 py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'analysis'
              ? 'border-emerald-600 dark:border-emerald-400 text-emerald-600 dark:text-emerald-400'
              : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'
            }`}
        ...
        <button
          ...
          className={`px-6 py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'reports'
              ? 'border-emerald-600 dark:border-emerald-400 text-emerald-600 dark:text-emerald-400'
              : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'
            }`}
                              ...
                              return (
                                <tr key={itemName} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <div className="flex items-center gap-4 mt-2 text-xs text-slate-600 dark:text-slate-300">
                                      ...
                                      {target > 0 && (
                                        <>
                                          ...
                                          {absDiffAmount > 1 && (
                                            <div className="flex items-center gap-1.5">
                                              <div className={`w-3 h-3 rounded-full border-2 border-dashed ${isUnderweight ? 'bg-emerald-400 dark:bg-emerald-500 border-emerald-600 dark:border-emerald-400' : 'bg-red-400 dark:bg-red-500 border-red-600 dark:border-red-400'
                                                }`} />
                                              <span>{isUnderweight
                                  ...
                                  <td className="py-4 px-4 text-center hidden md:table-cell">
                                    {absDiffAmount > 1 && target > 0 && (
                                      <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold ${isUnderweight
                ...
                return (
                  <>
                    ...
                    <div
                      ...
                      className={`px-3 py-2 rounded cursor-pointer transition-colors text-sm mb-2 ${selectedReport?.id === latestReport.id
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1 mt-2">
                        {otherReports.map((report) => (
                          <div
                            ...
                            className={`px-3 py-2 rounded cursor-pointer transition-colors text-sm ${selectedReport?.id === report.id
                                ? 'bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white font-medium'
                                : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'
                              }`}
                          >
